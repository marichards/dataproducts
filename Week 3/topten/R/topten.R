...
#' @return a vector of coefficients fromt he final fitted model with top 10 features
...
topten <- function(x,y){
  p <- ncol(x)
  if(p < 10)
    stop("Must supply at least 10 predictors")
...
predict10 <- function(X,b) {
  X <- cbind(1,X) # Add an intercept to X
  drop(X %*% b) # Drop to a vector; returns a vector of responses
}
